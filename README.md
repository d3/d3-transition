...
A transition is a [selection](https://github.com/d3/d3-selection)-like interface to manipulate the DOM smoothly over time, rather than instantaneously. To create a transition, first select some elements, and then call [*selection*.transition](#selection_transition). While transitions have similar methods to selections (such as [*transition*.attr](#transition_attr) and [*transition*.style](#transition_style)), selections and transitions function somewhat differently; for example, you must append elements using a selection before a transition starts, and [*transition*.remove](#transition_remove) waits to remove the selected elements until the transition ends.
